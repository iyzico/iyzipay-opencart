...
class iyzico extends Controller
{
    ...
    public function getCheckoutFormToken()
    {
        ...
        $payment_source = "OPENCART-4x-" . $this->moduleVersion . "|" . $this->moduleProductName;
        ...
        foreach ($products as $product) {
            ...
            if ($price) {
                ...
                $item->category1 = $this->model_extension_iyzico_payment_iyzico->getCategoryName($product['product_id']);
                $item->itemType = "PHYSICAL";
                $iyzico->basketItems[] = $item;
            }
        }
        ...
        if (!empty($shipping) && $shipping['cost'] && $shipping['cost'] != '0.00') {
            ...
            $iyzico->basketItems[$shippigKey]->name = $shipping['name'];
        ...
        $order_object = $this->model_extension_iyzico_payment_iyzico->createFormInitializObjectSort($iyzico);
        $pki_generate = $this->model_extension_iyzico_payment_iyzico->pkiStringGenerate($order_object);
        $authorization_data = $this->model_extension_iyzico_payment_iyzico->authorizationGenerate($pki_generate, $api_key, $secret_key, $rand_value);

        $iyzico_json = json_encode($iyzico, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);

        $response = $this->model_extension_iyzico_payment_iyzico->createFormInitializeRequest($iyzico_json, $authorization_data);

        $data['checkoutFormType'] = $this->config->get('payment_iyzico_design');
        $data['checkoutFormContent'] = $response->checkoutFormContent;
    ...
    public function getCallBack($webhook = null, $webhookPaymentConversationId = null, $webhookToken = null, $webhookIyziEventType = null)
    {
        ...
        try {
            ...
            $rand_value = rand(100000, 99999999);
            $pki_generate = $this->model_extension_iyzico_payment_iyzico->pkiStringGenerate($detail_object);
            $authorization_data = $this->model_extension_iyzico_payment_iyzico->authorizationGenerate($pki_generate, $api_key, $secret_key, $rand_value);
            ...
            if ($webhook == "webhook") {
                ...
                if ($webhookIyziEventType == 'CREDIT_PAYMENT_AUTH' && $request_response->paymentStatus == 'PENDING_CREDIT') {
                    ...
                    $this->model_checkout_order->addHistory($request_response->basketId, 1, $orderMessage);
                ...
                if ($webhookIyziEventType == 'CREDIT_PAYMENT_AUTH' && $request_response->status == 'success') {
                    ...
                    $this->model_checkout_order->addHistory($request_response->basketId, 2, $orderMessage);
                ...
                if ($webhookIyziEventType == 'CREDIT_PAYMENT_AUTH' && $request_response->status == 'FAILURE') {
                    ...
                    $this->model_checkout_order->addHistory($request_response->basketId, 7, $orderMessage);
                    return $this->webhookHttpResponse("Order Exist - Alışveriş kredisi işlemi başarısız sonuçlandı.", 200);
                }

                if ($request_response->paymentStatus == 'BANK_TRANSFER_AUTH' && $request_response->status == 'success') {
                    $orderMessage = 'iyzico Banka Havale/EFT ödemesi tamamlandı.';
                    $this->setWebhookText(0);
                    $this->model_checkout_order->addHistory($request_response->order_id, 5, $orderMessage);
            ...
            if (isset($request_response->cardUserKey)) {
                if ($customer_id) {
                    ...
                    if ($request_response->cardUserKey != $cardUserKey) {
                        $this->model_extension_iyzico_payment_iyzico->insertCardUserKey($customer_id, $request_response->cardUserKey, $api_key);
                    }
                }
            ...
            if ($installment > 1) {
                ...
                $this->model_extension_iyzico_payment_iyzico->orderUpdateByInstallement($iyzico_local_order->order_id, $request_response->paidPrice);
