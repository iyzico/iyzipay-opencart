<?php
	...
	class ControllerExtensionPaymentPaywithiyzico extends Controller
	{
		...
		public function index()
		{
			...
			$paywithiyzico->setCallbackUrl($this->url->link('extension/payment/paywithiyzico/getcallback', '', true));
			$paywithiyzico->setPaymentSource($payment_source);


			if ($paywithiyzico->getPaidPrice() == 0) {
				return false;
			}
			...
			foreach ($products as $key => $product) {
				...
				if ($price) {
					...
					$basketItem->setCategory1($this->model_extension_payment_paywithiyzico->getCategoryName($product['product_id']));
			...
			if (!empty($shipping) && $shipping['cost'] && $shipping['cost'] != '0.00') {
				...
				$basketItem->setName($shipping['title']);
			...
			$response = \Iyzipay\Model\PayWithIyzicoInitialize::create($paywithiyzico, $options);
			...
			$responseArray = [
				...
				'payWithIyzicoPageUrl' => $response->getPayWithIyzicoPageUrl(),
				'signature' => $response->getSignature(),
			];

			$data['pwi_redirect'] = $responseArray['payWithIyzicoPageUrl'];
		...
		public function getCallBack()
		{
			try {
				...
				$this->model_extension_payment_paywithiyzico->insertIyzicoOrder($paywithiyzico_local_order);


				if ($request_response->getPaymentStatus() != 'SUCCESS' || $request_response->getStatus() != 'success' || $order_id != $request_response->getBasketId()) {
					$errorMessage = $request_response->getErrorMessage() !== null ? $request_response->getErrorMessage() : $this->language->get('payment_failed');
					throw new \Exception($errorMessage);
				}
				...
				if ($request_response->getCardUserKey() !== null) {
					if ($customer_id) {
						...
						if ($request_response->getCardUserKey() != $cardUserKey) {
							$this->model_extension_payment_paywithiyzico->insertCardUserKey($customer_id, $request_response->getCardUserKey(), $api_key);
						}
					}
				}
				...
				if ($installment > 1) {
					...
					$this->model_extension_payment_paywithiyzico->orderUpdateByInstallement($paywithiyzico_local_order->order_id, $request_response->getPaidPrice());
